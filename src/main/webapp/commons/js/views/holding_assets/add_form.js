$(function() {
	...
	$(document).on("click", ".member_select_area li.member_li:not(.selected)",
			function() {
				console.log($(this));
	...
	$("#add_form_cancle").click(function() {
	...
	$("#submitBtn").on("click", function() {
		...
		if ($("#member_no").hasClass("added_input") == false) {
			swal("자산담당자를 선택해 주세요.");
			return flag;
		}
...
function removeSelectedClassOnSelectArea(memberNo) {
	...
	for (var i = 0; i < selectedMemberArr.length; i++) {
		...
		if (memberNo == currMemberNo) {
			// selected 제거
			$(
					".member_select_area li.member_li.selected[data-member-no="
							+ memberNo + "]").removeClass("selected");
		}
	}
}
...
function removeSelectedTagOnInputArea(memberNo) {
	...
	for (var i = 0; i < addedInputArr.length; i++) {
		...
		if (memberNo == currMemberNo) {
			// selected 제거
			$(".added_input_area input.added_input[value=" + memberNo + "]")
					.remove();
		}
	}
}
